...
case 57: {
	...
	case 72: 
	{
	if(!_shift && !_alt && !_ctrlKey && (currentWeapon player in ["hgun_P07_F","hgun_P07_snds_F"]) && (cursorTarget isKindOf "Car") && playerSide == west) then {
		[] call life_fnc_radar;
		};
	};
    ...
    case 39: {
		...
		if(playerSide in [civilian] && license_civ_VIP && vehicle player != player && !life_siren_active && ((driver vehicle player) == player)) then
		{
            [] spawn {
                life_siren_active = true;
                sleep 4;
            ...
            _veh = vehicle player;
            if (isNil {_veh getVariable "hdteam"}) then {_veh setVariable ["hdteam",false,true];};
            if ((_veh getVariable "hdteam")) then {
                titleText ["Hupe AUS","PLAIN"];
                _veh setVariable ["hdteam",false,true];
             } else {
                titleText ["Hupe AN","PLAIN"];
                _veh setVariable ["hdteam",true,true];
                if (playerSide isEqualTo civilian) then {
                   [_veh] remoteExec ["life_fnc_hdteam",RCLIENT];
                } else {
                    [_veh] remoteExec ["life_fnc_hdteam",RCLIENT];
                };
            };

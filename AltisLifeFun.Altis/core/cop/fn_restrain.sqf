...
if (side _cop isEqualTo civilian) then {
	[] spawn {
		...
		for "_i" from 0 to 1 step 0 do {
			...
			waitUntil {(time - _time) > (5 * 60)};
			if (!(player getVariable ["restrained",false])) exitWith {};
			if (!([civilian,getPos player,50] call life_fnc_nearUnits) && !([west,getPos player,50] call life_fnc_nearUnits) && (player getVariable ["restrained",false]) && isNull objectParent player) exitWith {
				player setVariable ["restrained",false,true];
				player setVariable ["Escorting",false,true];
				player setVariable ["transporting",false,true];
				detach player;
